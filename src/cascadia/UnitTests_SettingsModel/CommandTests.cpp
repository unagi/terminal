...
namespace SettingsModelUnitTests
{
    class CommandTests : public JsonTestClass
    {
        ...
        TEST_METHOD(HistoryToCommandsUsesDisplayWidthForBackspaces);
    };
    ...
    void CommandTests::HistoryToCommandsUsesDisplayWidthForBackspaces()
    {
        ...
        auto history = winrt::single_threaded_vector<winrt::hstring>();
        history.Append(L"echo done");

        const auto commands = Command::HistoryToCommands(history, L"\u2192", false, L"\ue81c");
        VERIFY_ARE_EQUAL(1u, commands.Size());

        const auto command = commands.GetAt(0);
        const auto args = command.ActionAndArgs().Args().try_as<SendInputArgs>();
        ...
        const std::wstring_view suffix{ L"echo done" };
        VERIFY_IS_TRUE(input.size() >= suffix.size());
        VERIFY_ARE_EQUAL(0, input.compare(input.size() - suffix.size(), suffix.size(), suffix.data()));
    }
}
